...
import org.gearman.GMServerFunctionMap;
...
final class ServerInstructionSet {
	...
	private final GMServerFunctionMap funcSet = new GMServerFunctionMap();
	/*
	 * Fwix Change
	 * Adding 3 FuncSet, one for each priority.
	 * Depending on the priority the job goes into the respective Priority set
	 */
	...
	public void getMemStats(final ServerClient client){
		...
		sb.append(Main.memUsed()); sb.append('\t');
		sb.append(Main.getThreadCount()); sb.append('\t');
		sb.append(Main.getActiveThreadCount()); sb.append('\t');
		sb.append(Main.getUpTime()); sb.append('\n');
	...
	public void update_throttle(final String[] args, final ServerClient client){//String function, Integer throttle){
		...
		if(args[2].equalsIgnoreCase("NOTHROTTLE")){
			...
			sb.append("remove throttle"); sb.append('\n');
		}
...
//		this.server.closePort(client.getLocalPort());
	...
	public class RemoveClass implements Runnable{
		...
		public RemoveClass(String funcName){
			this.function = funcName;
		}
		
		public void removeJobs(){
			Integer throttle = Main.getThrottle(function);
			
			if(throttle != null && throttle > 0){
				throttle = throttle * -1;
				Main.setThrottle(function, throttle);
			}
			ServerInstructionSet.this.removeJobs(function);
			
			throttle = Main.getThrottle(function);
			if(throttle != null && throttle < 0){
				throttle = throttle * -1;
				Main.setThrottle(function, throttle);
			}
